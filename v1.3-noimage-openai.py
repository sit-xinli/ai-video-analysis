    ...
    with tempfile.NamedTemporaryFile(suffix=".mp3", delete=True) as tmp:
        ...
        if has_speech(tmp.name):
            with open(tmp.name, "rb") as audio_file:
                    ...
                    # Simple heuristic
                    if len(transcript.text) < 5:  # or if it only contains noise-like text
                        print("⏩ Skipping: unclear speech")
...
def analyze_video(video_path):
    ...
    if video_path.lower().endswith(('.mp3', '.wav', '.flac')):
        audio_chunks = segment_audio(video_path, chunk_length_ms=interval_s * 1000)
        frames = extract_key_frames(video_path, interval_s)
        for i, chunk in enumerate(audio_chunks):
            print(f"Processing segment {i+1}/{len(audio_chunks)}...")
            transcript = transcribe_chunk(chunk)
            events = detect_events(chunk)
            scene = generate_scene_description(transcript, events, [""])
            results.append((i, transcript, events, "", scene))
    return results

results = analyze_video("bihar-road-traffic-304321.mp3")
