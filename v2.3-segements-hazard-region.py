...
def main_process(video_path, init_prompt, language, segments_of_video, frames_per_segment):
    ...
    if most_important_hazard_region and hazard_segment_index != -1:
        ...
        if frame_index < len(all_frames[hazard_segment_index]):
            ...
            box = most_important_hazard_region.get("box", [0,0,0,0])
            x = int(box[0] * img_width)
            y = int(box[1] * img_height)
            width = int(box[2] * img_width)
            height = int(box[3] * img_height)

            # Draw a thicker, brighter bounding box
            cv2.rectangle(img, (x, y), (x + width, y + height), (0, 255, 0), 3)
            label = f'{most_important_hazard_region.get("type", "Hazard")}: {most_important_hazard_region.get("score", 0.0):.2f}'
    ...
    return segmented_results_text, response, output_image, hazard_info
